...
/*
使用说明: 
    必须调用build方法传入相关参数后返回一个Mixin
参数说明(Object):
    *src(String): 播放的链接地址
    *duration(Number): 音频的总时长
    global(Boolean): 是否获取全局媒体对象
    autoStop: 页面隐藏/卸载时是否停止音频播放  默认自动停止播放
...
返回值说明(Mixin): 可用于mixins选项
*/

/* 初始化函数 */
function initMediaManager(options){
    if (options.global) options.mediaProxy.mediaManager = wx.getBackgroundmediaManager();
...
export default {
    build: function(options = {global: false,duration: -1,autoStop: true}){
        ...
        return {
            onHide(){
                if(options.autoStop)mediaProxy.mediaManager.stop();
            },
            onUnload(){
            ...
            methods: {
                ...
                mediaSeek(time){
                    if(time > options.duration)return console.warn("错误的进度跳转");
